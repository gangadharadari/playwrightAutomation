...
test('Fill DemoQA text box with JavaScript random data', async ({ page }) => {
  const randomValue = Math.random().toString(36).slice(2, 10);
  const testData = {
    fullName: `Random User ${randomValue}`,
    email: `random.${randomValue}@example.com`,
    currentAddress: `Current Address ${randomValue}`,
    permanentAddress: `Permanent Address ${randomValue}`,
  };
